...
function makeTower(col, row, typeId) {
  return {
    ...
    kills: 0,
    damageDealt: 0,
    muzzle: 0        // 발사 반동 연출 타이머
  };
}

// 레벨 + 전역 보너스가 반영된 실효 스탯
function towerStats(t) {
  const tpl = TOWER_TYPES[t.typeId];
  const m   = TOWER_LEVEL_MULT[t.level] || TOWER_LEVEL_MULT[1];
  return {
    ...
    slow:      tpl.slow ? Math.min(0.8, tpl.slow + BONUSES.towerSlowBonus) : 0,
    slowDur:   tpl.slowDur || 0,
    splash:    (tpl.splash || 0) + (tpl.splash ? BONUSES.towerSplashBonus : 0),
...
function makeDefenseEnemy(typeId, waveIndex) {
  ...
  const scale = 1 + Math.max(0, waveIndex) * DEF_WAVE_HP_SCALE;
  ...
  return {
    ...
    dmg: Math.round(tpl.dmg * (1 + Math.max(0, waveIndex) * 0.06)),
    reward: tpl.reward,
    armor: (tpl.armor || 0) + Math.floor(Math.max(0, waveIndex) / DEF_WAVE_ARMOR_EVERY),
    radius: tpl.radius,
