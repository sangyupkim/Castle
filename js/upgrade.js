...
const UPGRADE_CARDS = [
  ...
  { id:'t_cost',    name:'저렴한 자재',   desc:'타워 건설비용 -2',     grade:'common', icon:'🧱', cat:'tower',
    apply: b => { b.towerCostDiscount += 2; } },
  ...
  { id:'t_deepfrz', name:'극저온 냉각',   desc:'서리탑 감속 +20%p',    grade:'rare',   icon:'❄️', cat:'tower',
    apply: b => { b.towerSlowBonus += 0.2; } },
  { id:'t_bigboom', name:'고폭탄',        desc:'대포탑 폭발 반경 +18',  grade:'rare',   icon:'💣', cat:'tower',
    apply: b => { b.towerSplashBonus += 18; } },
  { id:'t_thunder', name:'천둥 화살',     desc:'모든 타워에 범위 피해 부여', grade:'epic', icon:'⚡', cat:'tower',
  ...
  { id:'t_arsenal', name:'대군 병기고',   desc:'타워 공격력 +5, 공속 +25%',  grade:'epic', icon:'🏹', cat:'tower',
    apply: b => { b.towerDmg += 5; b.towerSpdMult *= 1.25; } },
  ...
  { id:'u_hp1',     name:'강인한 체력',   desc:'아군 HP +15',          grade:'common', icon:'💪', cat:'unit',
    apply: b => { b.unitHp += 15; } },
  { id:'u_mana',    name:'마나 순환',     desc:'틱당 MP 회복 +3',      grade:'common', icon:'🔷', cat:'unit',
    apply: b => { b.mpRegenBonus += 3; } },
  { id:'u_rest',    name:'야영 기술',     desc:'웨이브 후 회복량 +20%p', grade:'common', icon:'🏕️', cat:'unit',
    apply: b => { b.restHealBonus += 0.2; } },
  ...
  { id:'u_crit',    name:'급소 노리기',   desc:'치명타 확률 +20% (1.8배)', grade:'rare', icon:'🎯', cat:'unit',
    apply: b => { b.critChance += 0.2; } },
  { id:'u_heal2',   name:'상급 치유술',   desc:'치유량 +15',           grade:'rare',   icon:'✚',  cat:'unit',
    apply: b => { b.healBonus += 15; } },
  { id:'u_shield2', name:'견고한 방벽',   desc:'보호막 +15',           grade:'rare',   icon:'🛡️', cat:'unit',
    apply: b => { b.shieldBonus += 15; } },
  ...
  { id:'u_legion',  name:'정예 군단',     desc:'슬롯 +1, 공격력 +4, HP +20', grade:'epic', icon:'🎖️', cat:'unit',
    apply: b => { b.maxSlotBonus += 1; b.unitAtk += 4; b.unitHp += 20; } },
  ...
  { id:'c_frail',   name:'취약한 껍질',   desc:'몬스터 HP -25%',       grade:'rare',   icon:'🗿', cat:'cave',
    apply: b => { b.mobHpMult *= 0.75; } },
  ...
  { id:'h_atk1',    name:'용기의 기운',   desc:'영웅 공격력 +5',       grade:'common', icon:'👑', cat:'hero',
    apply: b => { b.heroAtk += 5; } },
  { id:'h_regen',   name:'회복의 기운',   desc:'영웅 초당 HP +3 재생', grade:'common', icon:'👑', cat:'hero',
  ...
  { id:'b_regen',   name:'자동 수리반',   desc:'전투 중 초당 기지 HP +1', grade:'rare', icon:'🏰', cat:'base',
    apply: b => { b.baseRegen += 1; } },
  ...
  { id:'r_bounty',  name:'현상금 계약',   desc:'상단 처치 골드 +60%',  grade:'rare',   icon:'💰', cat:'resource',
    apply: b => { b.defenseGoldMult *= 1.6; } },
...
const META_UPGRADES = [
  ...
  { id:'m_kill_gold',  name:'처치 보너스',  icon:'💰', cat:'resource', maxLv:4, cost:15,
    desc: lv => `전투 골드 +${lv*10}%` },
  { id:'m_cave',       name:'케이브 전통',  icon:'🗿', cat:'resource', maxLv:2, cost:25,
    desc: lv => `케이브 Lv.${1+lv}부터 시작` },
];

// ─── 랜덤 카드 3장 뽑기 ──────────────────────────────────────────────────────
function rollUpgradeCards(taken) {
  const owned = new Set(taken || []);
  // 중복 불가 카드(불린/일회성 효과)는 이미 뽑았으면 제외
...
function applyMetaUpgrades(gs) {
  ...
  b.heroReviveReduction += lv('m_hero_revive') * 3;
...
// 메타로 해금된 시작 케이브 레벨
function startingCaveLevel(gs) {
  return Math.min(CAVE_MAX_LEVEL, 1 + (gs.metaUpgrades['m_cave'] || 0));
}
...
function calcSoulStones(gs) {
  ...
  const killBonus = Math.floor((gs.battle.runKills || 0) * 0.5);
  return Math.max(1, wavesCleared * 5 + hpBonus + caveBonus + killBonus);
}

// ─── 메타 업그레이드 구매 ─────────────────────────────────────────────────────
function metaUpgradeCost(upg, curLv) {
  return upg.cost * (curLv + 1); // 단계마다 비용 증가
}
